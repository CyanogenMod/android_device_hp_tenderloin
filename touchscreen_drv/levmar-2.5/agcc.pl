...
my $DROID = "/home/jon/beagle";
my $ABIBIN="$DROID/prebuilt/linux-x86/toolchain/arm-eabi-4.3.1/bin";
#my $GCC = `which arm-eabi-gcc`;
my $GCC="$ABIBIN/arm-eabi-gcc";
#$GCC = qx(cd `dirname $GCC`; /bin/pwd);
#chomp $GCC;
#die "bad arm-eabi-gcc path" if $GCC !~ /(.*)\/prebuilt\//;

my $ALIB = "$DROID/out/target/product/beagleboard/obj/lib";
...
my @include_paths = (
    ...
    "-I$DROID/out/target/product/dream/obj/include",
    ...
    "-I$DROID/bionic/libm",
    "-I$DROID/out/target/product/dream/obj/SHARED_LIBRARIES/libm_intermediates"
    );
...
my @link_libs = (
    "$TOOLCHAIN/lib/gcc/arm-eabi/4.3.1/libgcc.a",
    "-lc",
    "-lm");
...
@cmd = (@cmd, @args);
if($need_link) { @cmd = (@cmd, @link_libs); }
